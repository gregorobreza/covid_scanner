...
def qrEuValidityCheck(coviddata: str) -> bool:
    """
    ...
    if groupIdentifier == "r":
        validity, dateValid = recoveredCheck(covidListData)
    elif groupIdentifier == "v":
        validity, dateValid = vaccinatedCheck(covidListData)
    elif groupIdentifier == "t":
        validity, dateValid = testedCheck(covidListData)
    else:
    ...
    return validity, dateValid, names, birthday
...
def recoveredCheck(covidListData: str)-> bool:
    """
...
def vaccinatedCheck(covidListData: str)-> bool:
    """
...
def testedCheck(covidListData: str)-> bool:
    """
    ...
    if testType == "LP217198-3":
        #print("HAG test - veljavnost 48 ur")
        dateTimeValid = dateTimeTested + timedelta(hours=48)
    elif testType == "LP6464-4":
        #print("PCR test - veljavnost 72 ur")
        dateTimeValid = dateTimeTested + timedelta(hours=72)
    todayDate = datetime.today()
...
def identityCheck(namesFromQR: str, birthdayFromQR: str, namesLineID:str, birthdayLineID: str )-> bool:
    ...
    if (len(resNames) == len(namesFromQR)) and (birthdayFromQR in birthdayLineID):
        ...
        if len(resNames) != len(namesFromQR):
            ...
            print("Identiteta ni ustrezna. Napacno ime oz. priimek: QR: {} | ID:{}".format(namesFromQR, resNames))
            ...
            print("Identiteta ni ustrezna - rojstna datuma se ne ujemata: QR: {} | ID:{}".format(birthdayFromQR, birthdayLineID))
        identity = False
